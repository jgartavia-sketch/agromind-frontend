  ...
  const handleContactSubmit = async (event) => {
    ...
    if (subject.length < 3 || description.length < 10) {
      setContactStatus({
        type: "error",
        message:
          "Escribe un título de al menos 3 caracteres y una descripción de al menos 10.",
      });
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          data?.error || "No pudimos enviar la solicitud en este momento."
        );
      }
      ...
      setContactStatus({
        type: "error",
        message:
          error?.message || "No pudimos enviar la solicitud. Intenta de nuevo.",
      });
    }
  };
    ...
    <section className="shell-panel" aria-labelledby="support-title">
      ...
      >
        <div style={{ marginBottom: "20px" }}>
          ...
          <p style={{ margin: 0, color: "#5d6f60" }}>
            Completa el título y la descripción. Tu solicitud llegará
            directamente al equipo de AgroMind CR.
          </p>
        </div>
            ...
            style={{
              ...
              border:
                contactStatus.type === "error"
                  ? "1px solid rgba(185, 45, 45, .24)"
                  : contactStatus.type === "success"
                    ? "1px solid rgba(35, 125, 59, .24)"
                    : "1px solid rgba(55, 90, 64, .18)",
              background:
                contactStatus.type === "error"
                  ? "rgba(255, 237, 237, .92)"
                  : contactStatus.type === "success"
                    ...
                    : "rgba(242, 247, 242, .96)",
              color:
                contactStatus.type === "error" ? "#9c2828" : "#245c2d",
              fontWeight: 700,
            }}
          >
            {contactStatus.message}
        ...
        <button
          ...
          style={{
            ...
            fontWeight: 800,
            cursor:
              contactStatus.type === "loading" ? "wait" : "pointer",
            opacity: contactStatus.type === "loading" ? 0.72 : 1,
            boxShadow: "0 10px 24px rgba(34, 108, 49, 0.22)",
          }}
        >
